...
class Solution {
    /*
     * the interviewer never tells to not use extra space.
     * brute -> use an extra array, place all elements from both array in that array
     * and then sort the array using .sort() method and then copy the elements in
     * the original array updating it.
     * tc -> nlogn + n(for putting elements in new array) + n(for putting back in
     * original array)
     * sc -> n
     * 
     * better -> intuition based on that both the arrays we have is in sorted
     * fashion
     * hence if a1[i] > a2[0] then swap the elements. and then sort the second
     * array. at the last element of first array we would have the first array such
     * that if placed before second array, the group would be sorted.
     ...
     * the pointers have to move from one array to the other to complete the
     * iteration
